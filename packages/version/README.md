...
> Note that this will execute one or more remote API calls to the chosen remote server, which at the moment GitHub client is the only supported type. This option will also require a valid `GH_TOKEN` with read access permissions to the GitHub API so that it can execute the query to retrieve all commit details since the last release, for more info refer to the [`Remote Client Auth Tokens`](#--remote-client-auth-tokens).
